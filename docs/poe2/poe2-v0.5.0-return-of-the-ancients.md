# Path of Exile 2 - Return of the Ancients (v0.5.0)

## League Starter

### Viable

- [0.5 Ice Shot Deadeye Leveling Guide](https://mobalytics.gg/poe-2/profile/fubgun/builds/0-5-ice-shot-deadeye-leveling-guide?weaponSet=set1)
- Twisters?
- Monk?

### Got Nerfed

Poison Nerf:

- [Spiral Volley Pathfinder](https://maxroll.gg/poe2/build-guides/spiral-volley-poison-pathfinder)
- [Poisonburst Arrow Pathfinder](https://maxroll.gg/poe2/build-guides/poisonburst-arrow-pathfinder)

Shield Wall Nerf:

- [Shield Wall Warbringer](https://maxroll.gg/poe2/build-guides/shield-wall-warbringer-build-guide)

### Ranger

#### Gem Engraving Priority

| **Act** | **Active** | **Supports** | **Spirit** |
|---------|------------|--------------|------------|
| **Act I** | 1. Lightning Arrow | 1. Rapid Attacks I | Herald of Thunder |
|           |                    | 2. Elemental Armament I | 4. Concentrated Area |
|           | 1. Lightning Rod | 3. Concentrated Area | |
|           |                  | 5. Elemental Armament I | |
|           | Frost Bomb | | |
|           | Barrage | | |
|           | Ice-Tipped Arrows | | |
|           | Pounce | | |

#### Gear / Items

##### Act I

- Any plus flat damage (physical or elemental)
- Get a Shortbow in Red Vale (Act I)
- At lvl 11 get a Warden Bow, you're just looking for the highest DPS one you
  drop or buy from the vendor. **You're not looking to use any resources on this
  bow,** as you want to make a 6 mod Recurve Bow at lvl 16 with 2 sockets. So
  save your artificers orb, exalts, regals and alchemy orbs.
- Helmet/belt/amulet are good slots to acquire attributes (str on belt, dex or
  int on helmet)
- For the *Recurve Bow* look for high DPS, flat lightning damage and attack 
  speed. As Runes use *Lesser Storm Rune* and *Lesser Glacial Rune*. **Be
  careful to not use a Essence that is too high and making your bow unusable.